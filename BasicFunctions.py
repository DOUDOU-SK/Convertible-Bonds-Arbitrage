...
trade_dates = pd.read_csv('/home/ysli/ysli_codes/Raw/TradeDates/TRADINGDAYS.csv', index_col = 0)
...
LIMIT_UPs = pd.read_csv('/home/ysli/ysli_codes/Raw/Stock_Limit/Tickers_LIMITUP_TIME.csv')
...
class GetPickle:
    ...
    def get_every_date(self, trade_date_str):
        ...
        print(f'Successfully get {self.tickertype} data on {trade_date_str}')
        return temp_df     
...
class Factors:
    ...
    def get_factors(self, group):
        ...
        if self.FixTime: 
            ...
            ans['LIMIT_UP_TIME'] = LIMIT_UP_TIME
            ans['BREAK_LIMIT_UP_TIME'] = BREAK_LIMIT_UP_TIME
...
######################PART 17: GET Best Profits of every event =('Ticker', 'Date)

class BestProfits:

    def __init__(self, df_dict, limit_ups):
        self.df_dict = df_dict 
        self.limit_ups = limit_ups 

    def every_bond_profits(self, group):
        #### 假设这里的group是df和limit_ups合并后按照ticker分组的一个结果，是一个子dataframe 

        ds = group[(group['FunctionCode'] != 'C') & (group['TradeVolume'] >0)].copy()
        ds['date_time'] = pd.to_datetime(ds['date_time'])
        LIMIT_UP_TIME = pd.to_datetime(group['LIMIT_UP_TIME'].iloc[0])
        ds_after = ds[ds['date_time'] >= LIMIT_UP_TIME].copy()
        after_series = pd.Series(ds['date_time'] - LIMIT_UP_TIME)

        if not after_series.empty:
            ds_after.sort_values(by = 'date_time', inplace = True)
            
            if len(ds_after) > 0:
                entry_price = ds_after['TradePrice'].iloc[0]

                #### 找到最大收益时刻以及最大对数收益数值
                HighestPrice = ds_after['TradePrice'].max()
                HighestTime = ds_after.loc[ds_after['TradePrice'].idxmax(), 'date_time']
                HoldTime = (HighestTime - LIMIT_UP_TIME).total_seconds()
                #HighestTime = ds_after[da_after['TradePrice'] == HighestPrice]['date_time'].iloc[0]    
                HighestReturn_log = np.log(HighestPrice) - np.log(entry_price) 

                #### 找到最大回撤以及最大回撤发生的时刻
                ds_slice = ds_after[ds_after['date_time'] <= HighestTime].copy()
                ds_slice['peak'] = ds_slice['TradePrice'].cummax()
                ds_slice['drawdown'] = (ds_slice['peak'] - ds_slice['TradePrice']) / ds_slice['peak']
    
                ### 最大回撤与最大回撤发生的时刻
                max_dd = ds_slice['drawdown'].max()*100 # 把它转变成百分数
                max_dd_time = ds_slice.loc[ds_slice['drawdown'].idxmax(), 'date_time']

                ## 在最高点之前及之时，价格从未高于当前的峰值后再回落，因此没有形成相对峰值的回撤，drawdown 全部为 0
                ## 因此在全局峰值之前的时间段内，如果序列一直处于上升阶段，知道最后的HighestTime，那么drawdown为0是正常的
                #print("HighestTime:", HighestTime)
                #print("Peak at HighestTime:", ds_slice.loc[ds_slice['date_time'] == HighestTime, 'peak'].iloc[0])
                #print("Max drawdown value:", ds_slice['drawdown'].max())
                #print("Max drawdown time:\n", ds_slice.loc[ds_slice['drawdown'].idxmax(), ['date_time', 'TradePrice', 'peak']])

                ans = pd.DataFrame({
                    'HighestLogReturn': [HighestReturn_log],
                    'HighestTime': [HighestTime],
                    'EntryTime': [LIMIT_UP_TIME],
                    'HoldSeconds': [HoldTime],
                    'MaxDrawdown':[max_dd],
                    'MaxDrawdownTime': [max_dd_time],
                    'EntryPrice': [entry_price],
                    'HighestPrice': [HighestPrice]})
            else:
                print(f"No data after filter for ticker {group['Ticker'].iloc[0]}")
                ans = pd.DataFrame(columns=['HighestLogReturn', 'HighestTime', 'EntryTime', 'HoldSeconds', 'MaxDrawdown', 'MaxDrawdownTime', 'EntryPrice', 'HighestPrice'])        
    ...
    def every_date_profits(self, trade_date_str):

        dfs = self.df_dict[trade_date_str]
        self.limit_ups['Date'] = self.limit_ups['Date'].astype(str)
        dfs['Date'] = dfs['Date'].astype(str)
        dfs = dfs.merge(self.limit_ups, on = ['Date', 'Ticker'], how = 'left')
        #### 所以此时dfs是一个包含每个交易日内多个Ticker的Level-2交易数据的dataframe，还融合了每个事件的涨停时间

        result_df = dfs.groupby('Ticker').apply(lambda group: self.every_bond_profits(group)).reset_index() 
        if result_df.empty:
            print(f'No valid data about Best Profits on {trade_date_str}')
        else:
            print(f'Successfully get Best Profits on {trade_date_str}')

        return result_df 

    def get_all_profits(self):
        unique_dates = list(self.df_dict.keys())       

        resultx = Parallel(n_jobs=1)(
            delayed(self.every_date_profits)(date_str) for date_str in unique_dates
        )          
        resultx_dict = {
            date_str: res for date_str, res in zip(unique_dates, resultx) }

        result_profs = pd.concat(resultx_dict.values(), keys = resultx_dict.keys())
        result_profs = result_profs.reset_index(level=0).rename(columns={'level_0': 'Date'})
        #result_profs.rename(columns = {'level_0': 'Date'}, inplace = True)
        result_profs.drop(columns = ['level_1'], inplace = True)
...
class Factors_New:
    ...
    def robust_slope(self, x):
        ...
        # 3) 线性拟合，尽量捕获拟合中的异常
        try:
            # 自变量为序列索引（0..n-1），因变量为 x
            slope, intercept = np.polyfit(range(len(x)), x, 1)
            return slope
        except Exception:
            # 包括 LinAlgError 等，统一返回 NaN
            return np.nan

    def get_factors(self, group):
        """
        - group: the slice of transaction data group by  (the target date,'Ticker')
            one group is one event with unique ('Ticker', 'Date')
        """
        ################################ 初始化数据  ################################
        LIMIT_UP_TIME = pd.to_datetime(group['LIMIT_UP_TIME'].iloc[0])
        ...
        dt_ticker = group.copy() # .drop(columns = ['Ticker', 'LIMIT_UP_TIME', 'BREAK_LIMIT_UP_TIME'])
        dt = dt_ticker[(dt_ticker['FunctionCode'] != 'C') & (dt_ticker['TradeVolume'] > 0)].copy()
        dt['TradeAmount'] = dt['TradePrice'] * dt['TradeVolume']
        dt.sort_values(by = 'date_time', inplace = True)        
        ################################ 获取涨停时刻的基本特征数据  ################################
        ...
        ############ 2. 成交额特征 ############ 
        resampled['trade_amount_rolling'] = resampled['TradeAmount'].rolling(window=self.Win, min_periods=1).mean() #其中函数参数Win代表统一平滑的窗口长度
        resampled['trade_amount_growth'] = (np.log(resampled['trade_amount_rolling'])-np.log(resampled['trade_amount_rolling'].shift(1))).replace([np.inf, -np.inf], np.nan)
     
        ### 增加判断条件，这样避免产生虚假的大斜率
        #resampled['trade_amount_slope'] = resampled['TradeAmount'].rolling(window=self.Win, min_periods=2 ).apply(lambda x: np.nan if len(x) < 2 or np.allclose(x, x[0]) else  
        #            np.polyfit(range(len(x)), x, 1)[0] )

        resampled['trade_amount_slope'] = (resampled['TradeAmount'].rolling(window=self.Win, min_periods=2)  # 至少2个点才尝试拟合
                                .apply(lambda x: self.robust_slope(np.asarray(x, dtype=float))))            
        
        # 再计算斜率的差分，近似加速度
        resampled['amount_slope_growth'] = (np.log(resampled['trade_amount_slope'])-np.log(resampled['trade_amount_slope'].shift(1))).replace([-np.inf, np.inf], np.nan)
    
        ############ 3. 成交笔数特征 ############
        resampled['trade_count_rolling'] = resampled['TradeCount'].rolling(window = self.Win, min_periods=1).mean() 
        resampled['trade_count_growth'] = (np.log(resampled['trade_count_rolling'])-np.log(resampled['trade_count_rolling'].shift(1))).replace([np.inf, -np.inf], np.nan)
        # 先计算每个窗口的资金变动斜率
        #resampled['trade_count_slope'] = resampled['TradeCount'].rolling(window=self.Win, min_periods=2).apply((lambda x: np.nan if len(x) < 2 or np.allclose(x, x[0]) else  
        #            np.polyfit(range(len(x)), x, 1)[0] )) 
        resampled['trade_count_slope'] = (resampled['TradeCount'].rolling(window=self.Win, min_periods=2)  # 至少2个点才尝试拟合
                                .apply(lambda x: self.robust_slope(np.asarray(x, dtype=float))))
        
        resampled['count_slope_growth'] = (np.log(resampled['trade_count_slope'])-np.log(resampled['trade_amount_slope'].shift(1))).replace([-np.inf, np.inf], np.nan)

        ############ 4. 买卖力量特征 ############
        resampled['buy_ratio'] = resampled['BuyCount']/(resampled['BuyCount'] + resampled['SellCount']+1e-6) ##比值越接近1说明多方强，越接近0说明空方占优
        resampled['delta_buy_ratio'] = (np.log(resampled['buy_ratio'])-np.log(resampled['buy_ratio'].shift(1))).replace([-np.inf, np.inf], np.nan)
 
        ######### 5. 动态收益率及其相关特征，比如变化动量之类的 ###########
        resampled['log_return_3GAP'] = (np.log(resampled['TradePrice']) - np.log(resampled['TradePrice'].shift(3))).replace([np.inf, -np.inf], np.nan) # 原来是3
        resampled['momentum'] = resampled['log_return_3GAP'].rolling(window=self.Win, min_periods=1).mean() ######### momentum是log_return的平滑均值结果   
        diff_norm = resampled['momentum'].diff() / (resampled['momentum'].rolling(window=self.Win, min_periods=1).mean() + 1e-6)    
        resampled['momentum_decay_log'] = np.log1p(diff_norm)                  
        resampled['breakout_strength'] = (resampled['TradePrice'] - resampled['TradePrice'].rolling(window=self.Win, min_periods=1).max()) / resampled['TradePrice'].rolling(window=self.Win, min_periods=1).max()
        resampled['pullback_strength'] = (resampled['TradePrice'].rolling(window=self.Win, min_periods=1).max() - resampled['TradePrice']) / resampled['TradePrice'].rolling(window=self.Win, min_periods=1).max()
        resampled['hl_range'] = (resampled['TradePrice'].rolling(window=self.Win, min_periods=1).max() - resampled['TradePrice'].rolling(window=self.Win, min_periods=1).min()) / resampled['TradePrice']                         


        ######### 6. 订单簿特征 #########
        resampled['order_depth_diff'] = (resampled['BidOrder'] - resampled['AskOrder'])/(resampled['BidOrder'] + resampled['AskOrder']+1e-6)
        resampled['order_diff_growth'] = (np.log(resampled['order_depth_diff'])-np.log(resampled['order_depth_diff'].shift(1))).replace([-np.inf, np.inf], np.nan)
        resampled['order_pressure'] = resampled['AskOrder'] / (resampled['BidOrder'] + 1e-6)
        resampled['order_pressure_change'] = (np.log(resampled['order_pressure'])-np.log(resampled['order_pressure'].shift(1))).replace([-np.inf, np.inf], np.nan)
       
        
        ######### 7. 冲击成本 #########
        resampled['impact_cost'] = resampled['TradeAmount'] / (resampled['BidOrder'] + resampled['AskOrder'] + 1e-6)
        resampled['impact_cost_log'] = np.log1p(resampled['impact_cost'])

        ######### 8. 时间衰减特征 ###########
        #resampled['minutes_since_event'] = (pd.to_datetime(resampled.index) - pd.Timestamp(LIMIT_UP_TIME)).total_seconds() / 60.0
        #resampled['minutes_squared'] = resampled['minutes_since_event'] ** 2
    
        #############################
         
        cols = ['trade_amount_growth','trade_amount_slope', 'amount_slope_growth', 'trade_count_growth', 'trade_count_slope', 'count_slope_growth',
    
                'buy_ratio', 'delta_buy_ratio', 'momentum', 'momentum_decay_log', 'breakout_strength', 'pullback_strength','hl_range',
                
                'order_pressure', 'order_pressure_change', 'order_depth_diff', 'order_diff_growth','impact_cost_log'] # , 'minutes_since_event', 'minutes_squared'
        ...
        if self.FixTime:    
            ...
            dfs_tempt = dfs.dropna(how ='any')

            if dfs_tempt.empty:
                print('Type 1 Error, there is no data about resample')
                ans = pd.DataFrame(columns=['date_time'] + cols + ['special_time'])

            else:   
                try: 
                    limit_up_diff = dfs_tempt['date_time'] - LIMIT_UP_TIME
                    entry_idx_min = limit_up_diff.abs().idxmin()
                    ans = dfs_tempt.loc[[entry_idx_min], ['date_time'] + cols].copy()
                    ans['special_time'] = LIMIT_UP_TIME
                    ans = pd.DataFrame(ans)
        
                except (ValueError, KeyError, IndexError) as e:
                    print(f'Type 2 Error: {e}')
                    ans = pd.DataFrame(columns=['date_time'] + cols + ['special_time'])
            # 保证列结构一致
            expected_cols = ['date_time'] + cols + ['special_time']
            ans = ans.reindex(columns=expected_cols)
            return ans
...
#################### PART 19: 对数据进行归一化标准化处理
class ScaleProcess:

    def __init__(self, df, cols, methods, quantiles):
        self.df = df
        self.cols = cols 
    ...
    def scale_each_col(self, col, method,quantilex):
        # 使用副本，确保不修改原始 df 的其它列
        tempt = self.df.copy()
        # 处理缺失值：简单示例，按列填充均值
        if tempt[col].isnull().any():
            tempt[col] = tempt[col].fillna(tempt[col].mean())

        # 裁剪前先准备好分布范围（如需要）
        if quantilex is not None:
            lower = tempt[col].quantile(quantilex)
            upper = tempt[col].quantile(1 - quantilex)
            tempt[col] = tempt[col].clip(lower=lower, upper=upper)

        scaler = self._get_scaler(method)

        # 线性数据列应为 2D，reshape(-1,1)
        vals = tempt[[col]].values.reshape(-1, 1)
        scaled = scaler.fit_transform(vals)
        tempt[col] = scaled

        return tempt

    def scale_all_cols(self):
        # 开始时对 df 进行深拷贝，避免修改原始数据
        result = self.df.copy()
        for i in range(len(self.cols)):
            col = self.cols[i]
            method = self.methods[i]
            quantilex = self.quantiles[i]
            result = self._merge_scaled_col(result, col, method, quantilex)
        return result

    def _merge_scaled_col(self, base_df, col, method, quantilex):
        # 重新对指定列进行缩放，最终返回一个包含该列缩放结果的新 DataFrame
        scaled_df = self.scale_each_col(col, method, quantilex)
        # 只取缩放后的列，合并到 base_df
        base_df = base_df.copy()
        base_df[col] = scaled_df[col]
        return base_df      